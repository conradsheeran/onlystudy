...
- **专注模式**: 仅显示您选定的收藏夹、合集或 UP 主内容。过滤掉 Bilibili 主页的推荐流，拒绝信息干扰
- **安全锁定**: 支持密码锁定收藏夹配置。一旦锁定，必须输入密码才能修改显示的收藏夹，有效防止拖延和分心
- **高性能播放器**: 基于 `media_kit` 构建，提供流畅的硬件加速播放体验，支持倍速、后台播放与断点续播
- **双语界面**: 完整支持简体中文与英文，可跟随系统或手动切换
- **离线缓存**: 支持视频下载与离线播放（应用前台下载任务，含进度与失败重试）
  > 注：当前下载任务依赖应用在前台运行，并非系统级后台任务
...
- Flutter SDK (3.38.0 或更高版本；仓库 `.fvmrc` 固定 3.38.4，CI 使用同一版本)
- Android Studio / Xcode (移动端开发)
- Visual Studio / C++ Build Tools (Windows 开发)
- Linux 构建需额外安装：`clang cmake ninja-build pkg-config libgtk-3-dev liblzma-dev libstdc++-12-dev libasound2-dev libmpv-dev`
#### 2. 安装步骤
...
2.  选择内容: 登录后，勾选您希望在主页显示的收藏夹、合集或 UP 主
